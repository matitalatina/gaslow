...
describe("StationService", () => {
  ...
  it("should download and update DB with new data", () => {
    ...
    return service.updateStationCollection().then(() => {
  ...
  it("should find nearest stations given lat and lng", () => {
    ...
    sandbox
      .stub(Station, "findNearestByCoordinates")
      .resolves([returnedStation]);
    return service
      .findNearestByCoordinates(1.0, 2.0)
  ...
  it("should find by ids", () => {
    ...
    sandbox.stub(Station, "findByIds").resolves(returnedStations);
    return service.findByIds([1, 2]).then((stations: IStation[]) => {
      expect(stations).toEqual(returnedStations);
    });
  });
  ...
  it("should find on the route", async () => {
    ...
    when(mockGoogleMapsClient.getPolylineByRoute(from, to)).thenResolve(
      polyline,
    );
    when(mockGeoUtil.fromPolylineToPolygon(polyline)).thenReturn(
      featurePolygon,
    );
    const stubFindWithinPolygon = sandbox
      .stub(Station, "findWithinPolygon")
      .resolves([returnedStation]);
