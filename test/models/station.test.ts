...
describe('Station', () => {
  ...
  beforeEach(() => Station.remove({}).exec());

  it('should save and retrieve correctly', () => {
    const station = aStation();
    return station.save().then(() => Station.findOne({ id: station.id }).exec()
      .then((savedStation) => {
  ...
  it('should prevent two stations with same id', () => aStation().save()
    .then(() => aStation().save())
  ...
  it('should have mandatory fields', () => new Station({ prices: [{}] }).validate()
  ...
  it.skip('should work (full path)', () => StationService.updateStationCollection()
    .catch((err) => {
      console.log(err);
    }));
});
